...
export function EnvironmentLibrary() {
  ...
  const selected = environments.find((entry) => entry.id === selectedId) ?? sorted[0];
  ...
  return (
    <div className="environment-library">
        ...
        <div className="item-catalog-list">
          {sorted.map((environment) => (
            ...
            >
              ...
              <small>
                {ENVIRONMENT_LIGHTING_LABELS[environment.lighting]}
            ...
            <div className="item-editor-fields">
              <RamField label="Name" className="span-all">
                <RamInput
                  value={selected.name}
                  onChange={(event) =>
                    updateEnvironment(selected.id, { name: event.target.value })
                  }
                />
              </RamField>
              ...
              <RamField label="Notes" className="span-all">
                <RamTextarea
                  ...
                  onChange={(event) =>
                    updateEnvironment(selected.id, { notes: event.target.value })
                  }
              ...
              <div className="environment-map-actions">
                ...
                {selected.background && (
                  <RamIconButton
                    ...
                    variant="danger"
                    onClick={() => updateEnvironment(selected.id, { background: null })}
                  >
                ...
                <small>Characters are never stored or moved by environments.</small>
              </div>
              <input
                ref={fileRef}
                type="file"
                        ...
                        <div className={`token-row token-row--${kind}`} key={token.id}>
                          ...
                          <span className="token-row__actions">
                            <RamIconButton
                              ...
                              onClick={() =>
                                updateEnvironment(selected.id, {
                                  tokens: selected.tokens.filter((entry) => entry.id !== token.id),
                                })
                              }
                            >
      ...
      <RamConfirmDialog
        ...
        description="The current map, lighting, enemies, NPCs, and objects will be replaced. Characters stay where they are."
        confirmLabel="Apply"
        onConfirm={() => {
